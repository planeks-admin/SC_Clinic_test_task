...
- For managing Tasks items we use REST API due to it integration with Typescript/ autogenerated api client (
including validation, types etc)

- WS does not support typegeneration and not fully typed, so we use it as a channel to broadcast events, and perform 
actions on multiple clients

We use Ruff for the backend and Biome for the frontend — both are high-performance linters, considered among the fastest 
in their ecosystems, and support single-file configuration for easy setup and maintenance.
